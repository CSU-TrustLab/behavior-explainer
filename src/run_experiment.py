...
single behaviour (a cell in the prediction confusion matrix), each with a
...
3. Build or load sanity-checked behaviour data (confusion-matrix cells).
4. Filter to the requested behaviour (B, class_idx, other_class_idx).
5. For each of the 3 eraser types (Ortho, Splice, LEACE):
       run XPEnum  [→ optionally Saturate]  and  Exhaustive Search,
       each capped at TIMEOUT_SECS.
6. Write binary CSV results to results/<cm_name>/.

Notes
-----
- img_mean_map and img_mean_clip must exist in intermediate_results/ before
  running.  These are dataset-split statistics not yet generated by the
  pipeline (future work).
- Behaviour data is cached as intermediate_results/<cm_name>.pkl.  Pass
...
def make_behavior(behavior_num, class_idx, other_class_idx):
    """Return a behaviour predicate B(predicted, label) -> bool."""
    behaviors = {
...
def make_pred_fn(setup_b, align_inv, f_head,
...
def parse_args():
    ...
    p.add_argument(
        "--setup-a", default="f_enc",
        choices=["f_enc", "clip_enc", "f_space"],
        help="Embedding source (default: f_enc)",
    )
    p.add_argument(
        "--setup-b", default="f_head",
        choices=["f_head", "zs_clip", "f_space"],
        help="Prediction head (default: f_head)",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        metavar="1-6",
        help="Behaviour predicate number (default: 2 = correct predictions on class-idx)",
    )
    p.add_argument(
        "--class-idx", type=int, default=4,
        help="Primary class index for the behaviour predicate (default: 4)",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Max instances per behaviour per algorithm (default: 185)",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Depth for exhaustive search (default: 2)",
    )
    p.add_argument(
    ...
    p.add_argument("--no-explore",    action="store_true", help="Skip XPEnum")
    p.add_argument("--no-saturate",   action="store_true", help="Skip Saturate")
    p.add_argument("--no-exhaustive", action="store_true", help="Skip Exhaustive")
    p.add_argument(
        "--rebuild-behaviors", action="store_true",
        help="Force rebuild of behaviour data (ignore existing pkl)",
    )
    return p.parse_args()
...
def main():
    ...
    # ── Load image means ──────────────────────────────────────────────────────
    # TODO: generate these via a dedicated script (not yet migrated).
    #   img_mean_map  = mean of aligned embeddings over the training split
    #   img_mean_clip = mean of CLIP image embeddings over the training split
    img_mean_map  = Pickler.read(f"img_mean_map_{model_name}").to(device).to(torch.float64)
    img_mean_clip = Pickler.read(f"img_mean_clip_{dataset_name}").to(device).to(torch.float64)
    ...
    with torch.no_grad():
        if not cm_pkl.is_file() or args.rebuild_behaviors:
            ...
            print(f"+ Behaviour data saved: '{cm_name}.pkl'  ({len(all_data)} images)")

        else:
            print(f"\n── Loading behaviour data: {cm_name} ──")
    ...
    print(f"\nBehaviour '{behavior_suffix}': {len(filtered_data)} instances")

    if len(filtered_data) == 0:
        print("No instances match this behaviour — exiting.")
        return
    ...
    with torch.no_grad():
        for prefix, eraser_type in eraser_configs:
            ...
            if not args.no_explore:
                ...
                # --- Saturate (requires XPEnum results) ---
                if not args.no_saturate and xp_results is not None:
            ...
            if not args.no_exhaustive:
                if eraser_type == "leace":
                    eraser.keep_hashmap = True
                beh_id = f"{prefix}E{behavior_suffix}"
                try:
                    with time_limit(TIMEOUT_SECS):
                        wrapper_Exhaustive(
                            filtered_data, C_N, C_vectors_N, C_ord_signs, eraser, pred_fn,
